...
bool RGBToLuminanceNode::constant_fold(ShaderGraph *, ShaderOutput *, ShaderInput *optimized)
{
	if(inputs[0]->link == NULL) {
		optimized->set(linear_rgb_to_luminance(inputs[0]->value()));
		return true;
	}
...
{
	compiler.add(this, "node_convert_from_color2");
}
...
: ShaderNode("texture_coordinate")
{
	...
	add_output("WcsBox", SocketType::POINT);
	add_output("EnvSpherical", SocketType::POINT);
	add_output("EnvEmap", SocketType::POINT);
	add_output("EnvBox", SocketType::POINT);
	add_output("EnvLightProbe", SocketType::POINT);
	add_output("EnvCubemap", SocketType::POINT);
	add_output("EnvCubemapVerticalCross", SocketType::POINT);
	add_output("EnvCubemapHorizontalCross", SocketType::POINT);
	add_output("EnvHemi", SocketType::POINT);
...
static NodeEnum matrix_math_type_init()
{
	NodeEnum enm;
...
NodeEnum MatrixMathNode::type_enum = matrix_math_type_init();

MatrixMathNode::MatrixMathNode() : ShaderNode("matrix_math")
{
	type = (NodeMatrixMath)type_enum["Point"];
	tfm = transform_identity();
...
void MatrixMathNode::compile(SVMCompiler& compiler)
{
	...
	compiler.add_node(NODE_MATRIX_MATH, typeval, compiler.stack_assign(vector_in), compiler.stack_assign(vector_out));
